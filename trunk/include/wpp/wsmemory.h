 ...
 * 这个简单的 _vector 使用 memcopy 增长, T 必须是简单结构(即不能有指针)
 */
template <class T>
class _vector {
	T* ptr;
public:
	int number;						/// 当前数目
	int size;						/// 最大size
	void push_back(const T &t);		/// 向数组最后插入元素，内存会自动扩大
...
class WINDSOUL_API memory_stat {
	static int peak;				/// 使用时的最大值
	static int current;				/// 当前内存值
public:
 ...
 * memory<T> 中所记录的是T的指针
 *
 * memory_object, memory, memory::manager之间的关系是:
 *		momeory_object是记录内存对象的指针
 *		memory::manager作为memory的内嵌类, 提供对象内存的分配
 *		memory是整体上的管理类
 */
...
#endif
private:
	static memory_object* free_list;	/// 用于记录当前分配内存时memory_object的指针
	static manager pool;				/// 用于管理内存池
